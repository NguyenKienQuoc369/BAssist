...
import { Sparkles, Send, Paperclip, X, Database } from "lucide-react";
...
export default function Polisher() {
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      role: "assistant",
      content: "Xin chào! ✨ Mình là Polisher, chuyên gia về viết lách chuyên nghiệp. Mình sẽ giúp bạn biến bất kì đoạn text nào thành văn bản chuyên nghiệp, rõ ràng, và ấn tượng. Mình có thể cải thiện ngữ pháp, từ vựng, tone, và cấu trúc câu để phù hợp với bất kì ngữ cảnh nào - dù là email công việc, bài báo hay nội dung creative. Gửi cho mình đi nào! 💪"
    }
  ]);
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const response = await fetch(`${apiBase}/api/polisher`, {
        method: "POST",
        body: formData,
      ...
      const data = await parseResponseSafely(response);
      if (!response.ok || !data.success) {
        throw new Error(data?.detail || data?.error || "Không thể trau chuốt văn bản");
      }
      ...
      // Save to history
      addHistory("polisher", newMessages);
    } catch (err: any) {
          ...
          <div className="flex items-center justify-between">
            <div>
              <div className="flex items-center gap-2">
                ...
                }`}>Trau Chuốt Văn Bản</h1>
              </div>
              <p className={`text-xs mt-0.5 transition-colors duration-300 ${
                isDark ? "text-gray-400" : "text-gray-600"
              }`}>Biến văn bản thành Tiếng Anh chuyên nghiệp</p>
            </div>
          ...
          <div className="max-w-3xl mx-auto space-y-4 relative z-10">
            {messages.length === 0 && (
              <div className="text-center text-gray-400 py-20 animate-fade-up">
                ...
                <p className="text-sm font-medium">Gửi văn bản để trau chuốt</p>
              </div>
            )}
          ...
          <div className="max-w-3xl mx-auto">
            ...
            <div className="flex gap-2">
              ...
              <textarea
                ...
                placeholder="Nhập văn bản cần trau chuốt... (Enter gửi)"
